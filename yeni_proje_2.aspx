    ...
    function fiyatHesapla() {
        try {
            if (tryParseInt(document.getElementById('projects_campaign_prize').value)) {
                ...
                var ekSureBedeli = odul * (parseInt(document.getElementById('hfProjectTime').value) / 100);
                ...
                var genelToplam = odul + toplamHizmetBedeli + toplamEkstra + ekSureBedeli;
                odul = odul.toFixed(2);
                toplamHizmetBedeli = toplamHizmetBedeli.toFixed(2);
                toplamEkstra = toplamEkstra.toFixed(2);
                ekSureBedeli=ekSureBedeli.toFixed(2)
                ...
                document.getElementById('spanEkSureBedeli').innerHTML = ekSureBedeli.toString();
        ...
        if (tryParseInt(txtKaydiracFiyat.value)) {
            ...
            if(fiyat >= minFiyat && fiyat <= maxFiyat){
                ...
                for(var i=0; i<diziBalonlar.length; i++){
                    ...
                    diziBalonlar[i].innerHTML = txtKaydiracFiyat.value;
                }
                for(var i=0; i<diziBarlar.length; i++)
                    diziBarlar[i].style.width = (yeniKonum + balonGenislik) + 'px';

                document.getElementById('projects_campaign_prize').value = txtKaydiracFiyat.value;
                fiyatHesapla();
            }
        }
    }
...
<input runat="server" id="hfProjectTime" type="hidden" value="0" clientidmode="Static" />
...
<!-- FORM PROJE DETAY -->
    ...
    <div class='sum_information divTutar'>
        ...
        <p class='total_without_red_span' id='Xcommission_with_tax'>
            %25 markakafa hizmet bedeli + KDV: <span><span runat="server" id="spanResultServiceCost" clientidmode="Static">0</span> TL</span>
        </p>
        ...
        <p class='total_without_red_span' id='P1'>
            ek süre bedeli: <span><span runat="server" id="spanEkSureBedeli" clientidmode="Static">0</span> TL</span>
        </p>
